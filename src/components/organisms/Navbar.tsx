...
export default function Navbar() {
  ...
  return (
    <>
      {/* MOBILE MENU BUTTON - FIXED POSITION Z-100 */}
      {!isMenuOpen && !isFreebiesModalOpen && (
        <motion.button
          whileHover={{ scale: 1.1, rotate: 2 }}
          whileTap={WHILE_TAP}
          className="lg:hidden fixed top-5 right-5 text-black border-[3px] border-black bg-v2k-accent px-6 py-2 rounded-full shadow-[4px_4px_0px_#000] z-100 flex items-center justify-center cursor-pointer font-black"
          onClick={() => setIsMenuOpen(true)}
        >
          <Jersey tag="span" text="MENU" size="18|22" />
        </motion.button>
      )}
      ...
      <nav className="hidden lg:block w-full bg-nav-bg border-b-[3px] border-black relative z-40">
        <div className="flex items-center justify-between md:justify-center md:gap-8 lg:gap-16 px-4 py-3">
          ...
          <div className="hidden md:flex items-center gap-2 lg:gap-3">
            {[
              { ico: <IconInstagram className="w-5 h-5" />, bg: "bg-insta", hov: "hover:bg-pink-400", rotate: -8, href: "https://www.instagram.com/newjeans_official/" },
              { ico: <IconSpotify className="w-5 h-5" />, bg: "bg-spotify", hov: "hover:bg-green-500", round: "rounded-full", rotate: 8, href: "https://open.spotify.com/artist/6HvZYvR2fszIU3bGvbiTjC" },
              { ico: <IconX className="w-5 h-5" />, bg: "bg-nav-bg", hov: "hover:bg-blue-300", rotate: -5, href: "https://twitter.com/NewJeans_ADOR" },
              { ico: <IconFacebook className="w-5 h-5" />, bg: "bg-fb", hov: "hover:bg-blue-600", round: "rounded-full", txt: "text-white", rotate: 5, href: "https://www.facebook.com/official.newjeans/" },
            ].map((soc) => (
              <Link key={soc.href} href={soc.href} target="_blank">
                <motion.div
                  whileHover={{ y: -6, scale: 1.15, rotate: soc.rotate, boxShadow: "6px 6px 0px #000" }}
                  whileTap={WHILE_TAP}
                  transition={TRANSITION_SPRING}
          ...
          <motion.button
            whileHover={{ scale: 1.05, backgroundColor: "#000", boxShadow: "6px 6px 0px #ff69b4" }}
            whileTap={WHILE_TAP}
            transition={TRANSITION_SPRING}
          ...
          <div className="relative group">
            <Link href={user ? "/profile" : "/login"}>
              <motion.div
                whileHover={{ y: -6, scale: 1.15, rotate: -5, boxShadow: "6px 6px 0px #000" }}
                whileTap={WHILE_TAP}
                transition={TRANSITION_SPRING}
            ...
            {user && profile ? (
              ...
              >
                <span className="text-[8px] text-white font-black">LVL {Math.floor((profile.points || 0) / 100) + 1}</span>
              </motion.div>
            ) : (
              <motion.div
                initial={{ opacity: 0, x: 20 }}
              ...
              >
                <span className="text-[8px] text-black font-black">GUEST</span>
              </motion.div>
            )}
          </div>
        </div>
