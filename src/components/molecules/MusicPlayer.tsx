...
const HoloDisc = ({ size, hole }: { size: string; hole: string }) => (
  ...
  >
    ...
    >
      <div className="w-1/2 h-1/2 bg-v2k-dark border border-black rounded-full" />
    </div>
  </div>
);
            ...
            <div className="flex flex-col gap-4 px-1">
              <div className="flex justify-between items-center gap-3">
                <button
                  type="button"
                  onClick={playPrev}
                  className="flex-1 h-12 rounded-2xl bg-v2k-green-soft border-[3px] border-black text-black shadow-[4px_4px_0px_var(--color-v2k-black)] active:translate-x-1 active:translate-y-1 active:shadow-none transition-all flex items-center justify-center"
                >
              ...
              <div className="flex items-center justify-between gap-2 bg-white rounded-2xl p-2.5 border-[3px] border-black shadow-[4px_4px_0px_rgba(0,0,0,0.1)]">
                ...
                <div className="flex-1 flex flex-col items-center">
                  <SpaceText
                    text={`VOL: ${volume}%`}
                    size="12|12"
                    className="text-black/60 mb-1.5 font-bold"
                  />
                  <div className="flex gap-1">
                    {[20, 40, 60, 80, 100].map((v) => (
                      <div
                        key={`v-${v}`}
                        className={`w-3 h-2 rounded-sm border border-black ${volume >= v ? "bg-v2k-red-soft" : "bg-gray-100"}`}
                      />
                    ))}
                  </div>
                </div>
                <button
                  type="button"
                  onClick={() => adjustVol(10)}
                  className="w-9 h-9 rounded-xl bg-v2k-accent-soft border-2 border-black text-black font-black shadow-[2px_2px_0px_var(--color-v2k-black)] active:translate-y-0.5 flex items-center justify-center"
                >
                  <SpaceText text="+" size="12|12" />
                </button>
              </div>
