  ...
  return (
    <div className="mb-8 flex flex-wrap justify-center gap-4 sm:gap-8 bg-black/5 p-4 rounded-xl border-2 border-dashed border-black/20 backdrop-blur-sm">
      {FILTERS.map((filter) => (
        ...
        >
          <div
            className={`absolute inset-0 bg-black translate-x-1 translate-y-1 rounded-full transition-opacity ${currentFilter === filter.id ? "opacity-100" : "opacity-0 group-hover/pill:opacity-50"}`}
          />
          <div
            className={`relative px-6 py-2 rounded-full border-2 border-black flex items-center gap-2 transition-all 
            ${
          ...
          >
            <div
              className={`w-3 h-3 rounded-full border border-black ${currentFilter === filter.id ? "bg-white animate-pulse" : "bg-black/20"}`}
            />
            <SpaceText
              text={filter.label}
