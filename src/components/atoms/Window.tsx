...
export default function Window({ title, children, className = '', onClose }: WindowProps) {
  return (
    <div className={`bg-[#FFFCE0] border-[3px] border-black shadow-[4px_4px_0px_#000] overflow-hidden ${className}`}>
      {/* Barra de Título */}
      <div className="bg-[#CDE8FF] border-b-[3px] border-black px-2 py-1 flex justify-between items-center">
        <Jersey tag="span" text={title} size="14|14" className="text-black font-bold" />
        {onClose && (
          <motion.button
