...
export function AuthProvider({ children }: { children: React.ReactNode }) {
        ...
        const unsubProfile = onSnapshot(userDocRef, async (docSnap) => {
          if (docSnap.exists()) {
            ...
            const newProfile: UserProfile = {
              ...
              favMembers: [],
              favSongs: [],
              rank: CREATOR_EMAILS.includes(firebaseUser.email || "")
                ? "CREADOR / CEO"
                : "Bunny Novato",
              points: 0,
              streak: 1,
  ...
  const addPoints = useCallback(
    async (amount: number, message?: string) => {
      ...
      let activityText = "Puntos obtenidos";
      if (message) activityText = message;
      ...
      try {
        ...
        let currentPoints = 0;
        if (currentProfile.points) currentPoints = currentProfile.points;

        const newPoints = currentPoints + amount;
        const newRank = getRankByPoints(newPoints, userEmail);

        let currentActivities: Activity[] = [];
        if (currentProfile.activities)
          currentActivities = currentProfile.activities;
    ...
    [user],
  ); // Quitamos profile de dependencias ya que usamos getDoc
    ...
    if (user && profile) {
      ...
      if (lastLogin !== today) {
        setTimeout(async () => {
          ...
          const newStreak =
            lastLogin === yesterdayStr ? (profile.streak || 0) + 1 : 1;
          const xpBonus = newStreak >= 5 ? 40 : 20;
          const msg =
            newStreak >= 5
              ? `¡Racha de ${newStreak} días! 🔥 +${xpBonus} XP`
              : "Bonus por login diario 🐰";
  ...
  const uploadImage = async (file: File, subfolder: string = "avatars") => {
    ...
    try {
      const storageRef = ref(
        storage,
        `${subfolder}/${user.uid}/${Date.now()}_${file.name}`,
      );
